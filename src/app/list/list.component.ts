...
export class ListComponent implements OnInit {
  ...
  constructor(private componentService: ComponentservicesService, private location: Location, private router: Router, private route: ActivatedRoute, ) { 
  ...
  ngOnInit(): void {
    ...
    if(localStorage.getItem('access_token')){
        this.items = [];
        var currentItem;
        this.componentService.getitembylistbyid(this.id).subscribe(res =>{
            var count = Object.keys(res).length;
            for(var i=0; i<count; i++){
                currentItem = JSON.parse(res[i]['recipe']);
                currentItem['id'] = res[i]['id'];
  ...
  removeItem(id, index): void { //get index from template and pass it to service, remove item in service
    if(confirm("Are you sure to delete ?")) {
      ...
      // this.items = this.componentService.removeItem(index);
       if(localStorage.getItem('access_token')){
            this.componentService.removeItem(id).subscribe(res =>{
